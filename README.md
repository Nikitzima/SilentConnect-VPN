...
Инфраструктура построена на базе выделенных серверов (Hetzner, Aeza) под управлением Ubuntu/Debian. Сервис обеспечивает отказоустойчивое соединение и устойчивость к анализу трафика (DPI) за счёт использования современных мультипротокольных решений.
...
* **⚡ Быстрый** — VLESS + TCP + REALITY (с альтернативным SNI-профилем для повышения стабильности соединения)
* **🚀 Скоростной** — Hysteria2 (UDP/QUIC) для максимальной скорости потокового видео
* **🔐 Запасной** — VLESS + gRPC + REALITY (дополнительный fallback-профиль для повышения устойчивости соединения)
