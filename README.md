...
2. **Микросервис подписок (`subjson-service`)** — Написан на `Flask`. Динамически генерирует JSON-конфигурации для клиентов. Клиентские приложения автоматически стучатся на этот микросервис для получения актуальных настроек (IP серверов, ключи шифрования, порты).
...
4. **Ядро VPN** — `Xray-core` + `3X-UI`. Мультиплексирование портов настроено через `dokodemo-door`, что позволяет вешать на один 443 порт и обычный веб-трафик, и VPN-трафик.
