...
5. **GUI для развертывания нод** — Уникальный инструмент администратора, написанный на PowerShell (WinForms). Позволяет в пару кликов автоматически развернуть новую ноду (поднимает bash-скрипты, настраивает Xray/3X-UI, Caddy и ключи) на любом арендованном Ubuntu-сервере (Hetzner, Aeza).
6. **Автоматизация инфраструктуры (IaC)** — PowerShell-скрипты для автоматического развертывания обходных (ingress) серверов в Yandex Cloud через `yc cli` (VPC, Security Groups, статические IP).
