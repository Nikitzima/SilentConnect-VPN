...
HYSTERIA_SALAMANDER_PASSWORD = os.environ.get("HYSTERIA_SALAMANDER_PASSWORD", "06jhhep2msjmrfll").strip()
...
TCP_REALITY_PUBLIC_KEY = os.environ.get("TCP_REALITY_PUBLIC_KEY", "2JCWlJ5M8OxWsp8YLWZqtlkyJGMbdRSwVishk8mIuEo").strip()
TCP_REALITY_SHORT_ID = os.environ.get("TCP_REALITY_SHORT_ID", "9a2f7c6d1e4b8a30").strip()
...
GRPC_REALITY_PUBLIC_KEY = os.environ.get("GRPC_REALITY_PUBLIC_KEY", "qb3chneGBO60_kv-McyCvUrXM93aN851duFH2bVIMQU").strip()
GRPC_REALITY_SHORT_ID = os.environ.get("GRPC_REALITY_SHORT_ID", "4d7a").strip()
...
def build_four_profiles(
    ...
    # --- Profile 1: Sberbank Reality (TCP) ---
    sber_cfg = {
        ...
        "remarks": f"{SERVER_COUNTRY_FLAG} \U0001f6e1\ufe0f Классический ({email})",
        "meta": build_happ_config_meta(subscription_id),
        "routing": build_routing(route_mode),
        "stats": {}
    }
    sber_cfg["meta"]["serverDescription"] = "Классический \u00b7 TCP Reality"

    # --- Profile 2: Kinopoisk Reality (TCP) ---
    kino_cfg = {
        "log": {"loglevel": "warning"},
        "dns": {
            "queryStrategy": "UseIPv4",
        ...
        "remarks": f"{SERVER_COUNTRY_FLAG} \u26a1 Быстрый ({email})",
        "meta": build_happ_config_meta(subscription_id),
        "routing": build_routing(route_mode),
        "stats": {}
    }
    kino_cfg["meta"]["serverDescription"] = "Быстрый \u00b7 TCP Reality"

    # --- Profile 3: Hysteria 2 Salamander ---
    hyst_cfg = {
        "log": {"loglevel": "warning"},
        "dns": {
            "queryStrategy": "UseIPv4",
        ...
        "remarks": f"{SERVER_COUNTRY_FLAG} \U0001f680 Скоростной ({email})",
        "meta": build_happ_config_meta(subscription_id),
        "routing": build_routing(route_mode),
        "stats": {}
    }
    ...
    profiles = [sber_cfg, kino_cfg, hyst_cfg]

    # --- Profile 4: VLESS gRPC ---
    grpc_cfg = None
    nl_maxru_cfg = load_static_json_config("json-nl-maxru-xhttp")
    if nl_maxru_cfg:
        grpc_cfg = copy.deepcopy(nl_maxru_cfg)
        grpc_cfg["remarks"] = f"{SERVER_COUNTRY_FLAG} \U0001f510 Запасной ({email})"
        try:
            outbound = grpc_cfg["outbounds"][0]
            ...
            grpc_cfg["routing"] = build_routing(route_mode)
        ...
        if grpc_cfg:
            ...
            grpc_cfg["meta"]["serverDescription"] = "Запасной \u00b7 gRPC+Reality"

    # --- Profile 5: VLESS XHTTP H/3 ---
    xhttp_h3_cfg = None
    if nl_maxru_cfg:
        xhttp_h3_cfg = copy.deepcopy(nl_maxru_cfg)
        xhttp_h3_cfg["remarks"] = f"{SERVER_COUNTRY_FLAG} \U0001f30a Незаметный ({email})"
        try:
            outbound = xhttp_h3_cfg["outbounds"][0]
            outbound["settings"]["vnext"][0]["port"] = 28443
            outbound["settings"]["vnext"][0]["users"][0]["id"] = client_uuid
            outbound["settings"]["vnext"][0]["users"][0]["email"] = email
            outbound["settings"]["vnext"][0]["users"][0]["encryption"] = "none"
            if "tlsSettings" in outbound["streamSettings"]:
                outbound["streamSettings"]["tlsSettings"]["alpn"] = ["h3"]
            xhttp_h3_cfg["routing"] = build_routing(route_mode)
        except (KeyError, IndexError) as e:
            LOGGER.error(f"Error building NL XHTTP H3 profile: {e}")
            xhttp_h3_cfg = None

        if xhttp_h3_cfg:
            xhttp_h3_cfg["meta"] = build_happ_config_meta(subscription_id)
            xhttp_h3_cfg["meta"]["serverDescription"] = "Незаметный \u00b7 XHTTP HTTP/3"

    if grpc_cfg:
        profiles.append(grpc_cfg)
    if xhttp_h3_cfg:
        profiles.append(xhttp_h3_cfg)
...
def build_four_frankfurt_profiles(
    ...
    try:
        wifi_cfg = build_ws443_client_config(
            ...
            ws_host="109.120.176.75.sslip.io"
        )
      ...
      "outbounds": [
        {
          ...
          "streamSettings": {
            "finalmask": {
              "udp": [
                {"settings": {"password": "06jhhep2msjmrfll"}, "type": "salamander"}
              ]
            },
            "hysteriaSettings": {
              "auth": user_id,
            ...
            "tlsSettings": {
              "alpn": ["h3"],
              "fingerprint": "qq",
              "serverName": "edge.silentconnect.net"
            }
          },
          "settings": {
            "address": public_host,
        ...
        try:
            ...
            if len(path) == 3 and path[0] == SECRET_SEGMENT:
                ...
                if path[1] in {
                    ...
                    "sub-frankfurt-xhttp",
                    "json-frankfurt-hybrid",
                    "sub-frankfurt-hybrid",
                }:
                    payload = build_four_frankfurt_profiles(path[2])
                    if not payload:
                        self._send_json(
